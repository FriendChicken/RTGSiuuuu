...
class AutoTrader(BaseAutoTrader):
    ...
    def giveOrder(self):
        ...
        if self.bid_id == 0 and self.new_bid_price != 0 :
            ...
            self.send_insert_order(self.bid_id, Side.BUY, self.new_bid_price, min(50,POSITION_LIMIT-self.position), Lifespan.GOOD_FOR_DAY)
            self.bids.add(self.bid_id)

        if self.ask_id == 0 and self.new_ask_price != 0 :
            self.ask_id = next(self.order_ids)
            self.ask_price = self.new_ask_price
            self.send_insert_order(self.ask_id, Side.SELL, self.new_ask_price, min(50,POSITION_LIMIT+self.position), Lifespan.GOOD_FOR_DAY)
