...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        price levels.
        """
        HSLB_THRESHODE=int(0.1*TICK_SIZE_IN_CENTS)
        HSLB_STEP=int(0.2*TICK_SIZE_IN_CENTS)
        LIQ_THRESHODE=0*TICK_SIZE_IN_CENTS
        ...
        if instrument == Instrument.ETF:#
            ...
            if self.bid_prices[1][0]>=HSLB_THRESHODE+self.ask_prices[0][0]:
                #calculate lot size(My_lot_size)
                My_lot_size=LOT_SIZE+(self.bid_prices[1][0]-self.ask_prices[0][0])//HSLB_STEP
            ...
            elif self.ask_prices[1][0]+HSLB_THRESHODE<=self.bid_prices[0][0]:
                #calculate lot size(My_lot_size)
                My_lot_size=LOT_SIZE+(self.bid_prices[0][0]-self.ask_prices[1][0])//HSLB_STEP
                #if lot size is too much, buy maximum
                if self.position+My_lot_size>=POSITION_LIMIT:
            ...
            #Liquidify
            ...
            elif self.bid_prices[1][0]>=LIQ_THRESHODE+self.ask_prices[0][0] and self.position>0:
                #calculate lot size(My_lot_size)
                My_lot_size=LOT_SIZE
                #if lot size is too much, buy maximum
                if self.position-My_lot_size<0:
                    My_lot_size=self.position
                if self.ask_id != 0:
                    self.send_cancel_order(self.ask_id)
                    self.ask_id = 0
                self.ask_id = next(self.order_ids)
                ...
                self.logger.info("id %d , SELL ETF, position:%d",self.ask_id, self.position)
                self.asks.add(self.ask_id)
            elif self.ask_prices[1][0]+LIQ_THRESHODE<=self.bid_prices[0][0] and self.position<0:
                #calculate lot size(My_lot_size)
                My_lot_size=LOT_SIZE
                #if lot size is too much, buy maximum
                if self.position+My_lot_size>0:
