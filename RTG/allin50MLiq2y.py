...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.FUTURE:
            ...
            new_bid_price = bid_prices[0] -200  if bid_prices[0] != 0 else 0
            new_ask_price = ask_prices[0] +200  if ask_prices[0] != 0 else 0
            ...
            if self.bid_id == 0 and new_bid_price != 0 and self.position<50:
                if self.position >-40:
                    self.bid_id = next(self.order_ids)
                    ...
                    self.bid_id = next(self.order_ids)
                    self.bid_price = new_bid_price
                    self.send_insert_order(self.bid_id, Side.BUY, bid_prices[0]-100, min(50,POSITION_LIMIT-self.position), Lifespan.GOOD_FOR_DAY)
                    self.bids.add(self.bid_id)

               
            if self.ask_id == 0 and new_ask_price != 0 and self.position>-50:
                if self.position < 40:
                    self.ask_id = next(self.order_ids)
                    ...
                    self.ask_id = next(self.order_ids)
                    self.ask_price = new_ask_price
                    self.send_insert_order(self.ask_id, Side.SELL, ask_prices[0]+100, min(50,POSITION_LIMIT+self.position), Lifespan.GOOD_FOR_DAY)
                    self.asks.add(self.ask_id)
