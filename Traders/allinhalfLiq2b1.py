...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.FUTURE:
            price_adjustment = - (self.position // 25) * TICK_SIZE_IN_CENTS
            self.new_bid_price = bid_prices[0] -200 +price_adjustment if bid_prices[0] != 0 else 0
            self.new_ask_price = ask_prices[0] +200 +price_adjustment if ask_prices[0] != 0 else 0

            if self.bid_id != 0 and self.new_bid_price not in (self.bid_price, 0):
                self.send_cancel_order(self.bid_ids[0])
                self.send_cancel_order(self.bid_ids[1])
                self.bid_ids[0] = 0
                self.bid_ids[1] = 0
            if self.ask_id != 0 and self.new_ask_price not in (self.ask_price, 0):
                self.send_cancel_order(self.ask_ids[0])
                self.send_cancel_order(self.ask_ids[1])
                self.ask_ids[0] = 0
                self.ask_ids[1] = 0

            if self.bid_ids[0] == 0 and self.bid_ids[1] == 0 and self.new_bid_price != 0 :
                if self.position >-60:
                    if self.position <30:
                        self.bid_id = next(self.order_ids)
                        self.logger.info("send buy order with id %d, price %d,lot %d", self.bid_id, self.new_bid_price,tmp_lot)
                        self.send_insert_order(self.bid_id, Side.BUY, self.new_bid_price, (POSITION_LIMIT-self.position)//3 , Lifespan.GOOD_FOR_DAY)
                        self.bid_price = self.new_bid_price
                        self.bids.add(self.bid_id)
                        self.bid_ids[0]= self.bid_id
                        self.bid_id = next(self.order_ids)
                        self.logger.info("send buy order with id %d, price %d,lot %d", self.bid_id, self.new_bid_price,tmp_lot)
                        self.send_insert_order(self.bid_id, Side.BUY, self.new_bid_price+100, (POSITION_LIMIT-self.position)//6 , Lifespan.GOOD_FOR_DAY)
                        self.bid_price = self.new_bid_price
                        self.bids.add(self.bid_id)
            ...
            if self.ask_ids[0] == 0 and self.ask_ids[1] == 0  and self.new_ask_price != 0 :
                if self.position <60:
                    if self.position >-30:
                        self.ask_id = next(self.order_ids)
                        self.logger.info("send sell order with id %d, price %d,lot %d", self.ask_id, self.new_ask_price,tmp_lot)
                        self.send_insert_order(self.ask_id, Side.SELL, self.new_ask_price, (POSITION_LIMIT+self.position)//3, Lifespan.GOOD_FOR_DAY)
                        self.ask_price = self.new_ask_price
                        self.asks.add(self.ask_id)
                        self.ask_ids[0] = self.ask_id 
                        self.ask_id = next(self.order_ids)
                        self.logger.info("send sell order with id %d, price %d,lot %d", self.ask_id, self.new_ask_price,tmp_lot)
                        self.send_insert_order(self.ask_id, Side.SELL, self.new_ask_price-100, (POSITION_LIMIT+self.position)//6, Lifespan.GOOD_FOR_DAY)
                        self.ask_price = self.new_ask_price
                        self.asks.add(self.ask_id)
    ...
    def on_order_status_message(self, client_order_id: int, fill_volume: int, remaining_volume: int,
        ...
        if remaining_volume == 0:
            if client_order_id == self.bid_ids[0]:
                self.bid_ids[0] = 0
            if client_order_id == self.bid_ids[1]:
                self.bid_ids[1] = 0
            if client_order_id == self.ask_ids[0]:
                self.ask_ids[0] = 0
            if client_order_id == self.ask_ids[1]:
                self.ask_ids[1] = 0
