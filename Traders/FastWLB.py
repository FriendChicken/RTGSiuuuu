...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        PRICETHRESHOLD=int(0.1*TICK_SIZE_IN_CENTS)
        ...
        if instrument == Instrument.ETF:
            if self.bid_prices[1][0]>=PRICETHRESHOLD+self.ask_prices[0][0]:
                ...
                self.logger.info("id %d , SELL ETF for price %d, position:%d",self.ask_id, self.bid_prices[1][0], self.position)
                self.asks.add(self.ask_id)
            elif self.ask_prices[1][0]+PRICETHRESHOLD<=self.bid_prices[0][0]:
                #calculate lot size(My_lot_size)
