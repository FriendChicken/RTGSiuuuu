...
class AutoTrader(BaseAutoTrader):
    ...
    def spreadCal(self, gamma, var, T, t, k):
        return gamma*var*(t)/T+2/gamma*np.log(1+gamma/k)


    def midpriceCal(self,s,q, gamma, var, T, t, k):
        return s-q*gamma*var*(t)/T
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.ETF:
            ...
            price_adjustment = self.spreadCal(self.gamma,self.variance,900,time.time()-self.startTime,self.k)
            print("price_adjustment=",price_adjustment)
            midprice = self.midpriceCal(((self.my_ask_prices[0][0]+self.my_bid_prices[0][0])/2)/100,self.position/POSITION_LIMIT,self.gamma, self.variance, LIQ_TIME, time.time()-self.startTime, self.k)
            print("(self.my_ask_prices[0][0]+self.my_bid_prices[0][0])//2=",(self.my_ask_prices[0][0]+self.my_bid_prices[0][0])//2)
