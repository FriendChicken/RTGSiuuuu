...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.FUTURE:
            ...
            if self.bid_id == 0 and new_bid_price != 0 and self.position<60:
                if self.position >-40:
                    self.bid_id = next(self.order_ids)
                    self.bid_price = new_bid_price
                    self.send_insert_order(self.bid_id, Side.BUY, new_bid_price, min(20,POSITION_LIMIT-self.position), Lifespan.GOOD_FOR_DAY)
                    self.bids.add(self.bid_id)
                else:
                    self.bid_id = next(self.order_ids)
                    self.bid_price = new_bid_price
                    self.send_insert_order(self.bid_id, Side.BUY, bid_prices[0], min(20,POSITION_LIMIT-self.position), Lifespan.GOOD_FOR_DAY)
                    self.bids.add(self.bid_id)

               
            if self.ask_id == 0 and new_ask_price != 0 and self.position>-60:
                if self.position < 40:
                    self.ask_id = next(self.order_ids)
                    self.ask_price = new_ask_price
                    self.send_insert_order(self.ask_id, Side.SELL, new_ask_price, min(20,POSITION_LIMIT+self.position), Lifespan.GOOD_FOR_DAY)
                    self.asks.add(self.ask_id)
                else:
                    self.ask_id = next(self.order_ids)
                    self.ask_price = new_ask_price
                    self.send_insert_order(self.ask_id, Side.SELL, ask_prices[0], min(20,POSITION_LIMIT+self.position), Lifespan.GOOD_FOR_DAY)
                    self.asks.add(self.ask_id)
